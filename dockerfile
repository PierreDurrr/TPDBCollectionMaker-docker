...
RUN -c apt install software-properties-common; \
RUN -c add-apt-repository ppa:ubuntu-toolchain-r/test; \
RUN -c apt-get update; \
RUN -c apt-get install -y gcc; \
RUN -c apt-get install -y git; \
